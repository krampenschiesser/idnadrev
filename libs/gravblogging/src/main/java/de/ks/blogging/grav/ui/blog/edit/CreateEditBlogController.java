...
public class CreateEditBlogController extends BaseController<GravBlog> {
  ...
  public void initialize(URL location, ResourceBundle resources) {
    ...
    validationRegistry.registerValidator(defaultAuthor, new NotEmptyValidator(localized));
    validationRegistry.registerValidator(pagesDir, new NotEmptyValidator(localized));
    validationRegistry.registerValidator(pagesDir, new FileExistsValidator(localized));
    validationRegistry.registerValidator(blogSubPath, new NotEmptyValidator(localized));
    validationRegistry.registerValidator(blogSubPath, new FileExistsValidator(localized) {
      @Override
      protected String getFilePathFromString(String s) {
        return new File(pagesDir.getText(), s).getPath();
      }
    });
    validationRegistry.registerValidator(imageDimension, new NotEmptyValidator(localized));
    validationRegistry.registerValidator(imageDimension, new IntegerRangeValidator(localized, 320, 10000));
